...
import { APIFY_TOKEN, LINKEDIN_SCRAPER_ACTOR_ID, URL_REGEX, ERROR_MESSAGES } from '../config.js';
export class LinkedInScraper {
    ...
    transformExperiences(experiences) {
        ...
        return experiences.map(exp => {
            ...
            if (exp.breakdown && Array.isArray(exp.subComponents)) {
                mainPosition.subPositions = exp.subComponents.map((sub) => ({
    ...
    transformEducation(education) {
        ...
        return education.map(edu => {
            ...
            const activities = descriptions.find((desc) => desc.text?.includes('Activities and societies:'))?.text?.replace('Activities and societies: ', '');
            const grade = descriptions.find((desc) => desc.text?.includes('Grade:'))?.text?.replace('Grade: ', '');
            return {
    ...
    transformSkills(skills) {
        ...
        return skills.map(skill => {
            const endorsements = skill.subComponents?.[0]?.description
                ?.filter((desc) => desc.type === 'insightComponent')
                ?.map((desc) => desc.text);
            const endorsementCount = endorsements?.find((text) => text.includes('endorsements'))
                ?.match(/(\d+) endorsements?/)?.[1];
            const endorsedByInfo = endorsements?.find((text) => text.includes('Endorsed by'))
    ...
    transformUpdates(updates) {
        ...
        return updates.map(update => ({
            ...
            reactions: update?.reactionTypeCounts?.map((reaction) => ({
    ...
    async extractProfile(profileUrl) {
        try {
